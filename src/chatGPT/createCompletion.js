    ...
    promise: new Promise((resolve, reject) => {
      fetch(API_URL, {
        ...
        fetchStream(
          ...
          result.ok ? resolve({ content, role: "assistant" }) : reject(content);
        });
      });
    }),
  };
    ...
    for (const entry of data.split("\n"))
      if (entry) {
        ...
        if (response && typeof response.choices[0].delta.content === "string") {
          if (typeof acc === "string") acc += response.choices[0].delta.content;
...
function errorParser(data, acc) {
  ...
  if (typeof acc.message === "string") acc.message += parsed.error.message;
